...
public class Pallet : MonoBehaviour
{
    ...
    void InnerMoveTo(WayPoint p)
    {
        ...
        if (p.name == "i-1-1-1" || p.name == "i-2-1-1")
        {
    ...
    */
        ...
        if (current != null || (current == null && step == 10))
        {
            switch (step)
            {
                case 0:
                    ...
                    this.transform.parent = null;
                    step = 1;
                    this.speed = (target - this.transform.position).magnitude / G.PalletSpeed;

                    break;
                case 1:
                    var p = this.transform.position;
                    var tt = Vector3.MoveTowards(p, target, (output_liu ? G.PalletAutoSpeed : G.PalletSpeed) * Time.deltaTime);
    ...
    public void SendPoint(WayPoint p, bool force = false)
    {
        ...
        if (p == null || removed)
            return;
        if (p.name[0] == 'h' || p.name[0] == 'i' || p.name[0] == 't')
            return;
        if (p.name == "01035")
        {
